...
// Prevent two running copies from fighting over Chromium's Cache/GPUCache on
// Windows. That exact fight shows up as "Unable to move/create cache: Access
// denied (0x5)" and can also make preload diagnostics very noisy.
...
type StudioSettings = Record<string, any>;

async function loadSettings(): Promise<StudioSettings> {
  return await readJson<StudioSettings>(SETTINGS_FILE, { theme: 'pulse', python: pythonCmd() });
}

async function saveSettingsPatch(patch: StudioSettings) {
  const current = await loadSettings();
  await writeJson(SETTINGS_FILE, { ...current, ...patch });
}

function splitSetCookieHeader(value: string) {
  // Undici may expose Set-Cookie as one comma-joined header. A comma inside
  // Expires must not split the cookie, so split only before a new name=value.
  return value.split(/,(?=\s*[^;,\s]+=)/g).map(x => x.trim()).filter(Boolean);
}

function getSetCookieHeaders(headers: any): string[] {
  if (!headers) return [];
...
async function rememberAuthCookieFromResponse(response: Response) {
  const setCookies = getSetCookieHeaders((response as any).headers);
...
function filenameFromUpdate(manifest: any) {
...
async function downloadInnoUpdate(manifest: any) {
  ...
  await fs.writeFile(dest, Buffer.concat(chunks), { mode: 0o600 });
...
async function installDownloadedInnoUpdate(installerPath?: string) {
  const settings = await loadSettings();
...
async function pulseApi(pathname: string, options: { method?: string; body?: any; auth?: boolean } = {}) {
  const settings = await loadSettings();
...
function cleanNullableUrl(value: any) {
  const s = String(value ?? '').trim();
  return s ? s : null;
}

function cleanAuthorProfilePayload(payload: any) {
  return {
...
async function readMarketplaceStats() {
  const read = async (name: string, pathname: string) => {
  ...
  const errors = [installations, developerTokens, ledger, products].filter((x: any) => !x.ok);
  return {
...
async function scanProjectSecurity(projectId: string) {
  ...
  const skipDirs = new Set(['.venv', 'node_modules', '__pycache__', '.git', 'dist', '.plugin-studio']);
  const textExt = new Set(['.py','.json','.md','.txt','.yml','.yaml','.toml','.ini','.env','.js','.ts','.tsx','.jsx','.html','.css']);
...
const REVIEW_EXCLUDED_DIRS = new Set([
  '.venv', 'node_modules', '__pycache__', '.git', '.idea', '.vscode',
  'dist', 'build', 'out', 'coverage', '.plugin-studio', '.pytest_cache',
...
function shouldExcludeFromReviewPackage(name: string) {
  if (REVIEW_EXCLUDED_DIRS.has(name)) return true;
  return REVIEW_EXCLUDED_FILES.some(r => r.test(name));
}

async function packageGuard(projectId: string) {
  const { actualRoot } = await getProject(projectId);
  const excluded: any[] = [];
  const included: any[] = [];
      ...
      if (shouldExcludeFromReviewPackage(entry.name)) {
        excluded.push({
          ...
          reason: entry.isDirectory()
            ? 'служебная/build/cache папка не должна попадать в review-пакет'
            : 'служебный архив/лог/секретный или временный файл не должен попадать в review-пакет'
        });
        continue;
      }
      if (entry.isDirectory()) {
  ...
  return {
    ...
    hint: missing.length
      ...
      : excluded.length
        ? 'Studio опубликует через временную clean-staging папку: лишние файлы будут исключены из review-пакета.'
        : 'Лишних файлов для review-пакета не найдено.'
  };
}

async function copyCleanProjectForReview(srcRoot: string, destRoot: string) {
  await ensureDir(destRoot);
  async function copy(currentSrc: string, currentDest: string) {
...
async function runPulseUsingVenv(venvProjectRoot: string, cwd: string, args: string[], env: NodeJS.ProcessEnv = {}, timeoutMs = 120000) {
...
const COMMON_IMPORT_PACKAGES: Record<string, string> = {
  requests: 'requests',
  aiohttp: 'aiohttp',
  httpx: 'httpx',
  bs4: 'beautifulsoup4',
...
async function scanPythonImports(projectRoot: string) {
  const found = new Set<string>();
  const skipDirs = new Set(['.venv', '__pycache__', '.git', '.plugin-studio', 'dist', 'build', 'node_modules']);
  async function visit(dir: string) {
    for (const entry of await fs.readdir(dir, { withFileTypes: true })) {
      if (entry.isDirectory()) {
...
async function moduleAvailable(py: string, moduleName: string, cwd: string, env: NodeJS.ProcessEnv) {
  const r = await spawnCollect(py, ['-c', `import ${moduleName}`], cwd, env, 30000);
  return r.success;
}

async function ensureProjectRuntimeDependencies(projectId: string, actualRoot: string, sslEnv: NodeJS.ProcessEnv = {}) {
  ...
  if (await exists(requirements)) {
    const reqInstall = await spawnCollect(py, ['-m', 'pip', 'install', '-r', requirements], actualRoot, sslEnv, 240000);
    ...
    if (!reqInstall.success) {
      return {
        ...
        steps,
        hint: (reqInstall.stderr || '').includes('CERTIFICATE_VERIFY_FAILED')
          ? 'pip не смог скачать зависимости из-за SSL. Нажми “Исправить SSL / certifi” и повтори test/doctor.'
          : 'Не удалось установить зависимости из requirements.txt. Проверь stderr выше.'
      };
    }
  }
  ...
  if (uniquePackages.length) {
    const autoInstall = await spawnCollect(py, ['-m', 'pip', 'install', ...uniquePackages], actualRoot, sslEnv, 240000);
...
function shouldRunInCleanStaging(preset: string) {
  return ['validate', 'check', 'test', 'doctor', 'pack', 'dry-run', 'run-fixtures'].includes(preset);
}

function shouldEnsureRuntimeDeps(preset: string) {
...
async function appendSessionEvent(projectId: string, event: any) {
  try {
    const { actualRoot } = await getProject(projectId);
    ...
    // Session logs must never break SDK commands.
  }
}

async function listSessionReports(projectId: string) {
  const { actualRoot } = await getProject(projectId);
  const dir = path.join(actualRoot, '.plugin-studio', 'sessions');
...
async function walk(root: string, dir = root): Promise<any[]> {
  const items: any[] = [];
...
function pythonLaunchers() {
  return process.platform === 'win32'
    ? [{ cmd: 'python', args: [] as string[] }, { cmd: 'py', args: ['-3'] }, { cmd: 'python3', args: [] as string[] }]
    : [{ cmd: 'python3', args: [] as string[] }, { cmd: 'python', args: [] as string[] }];
}

async function createVenv(cwd: string) {
  let last: any = null;
  for (const launcher of pythonLaunchers()) {
    const r = await spawnCollect(launcher.cmd, [...launcher.args, '-m', 'venv', '.venv'], cwd, {}, 120000);
...
function sanitizePluginId(value: string) {
...
async function repairLegacyManifest(projectRoot: string) {
  const manifestPath = path.join(projectRoot, 'funpay-pulse.plugin.json');
  const manifest = await readJson<any>(manifestPath, null as any);
  if (!manifest || typeof manifest !== 'object') return null;

  let changed = false;
  if (!manifest.manifest_version) { manifest.manifest_version = '1.0'; changed = true; }
  ...
  if (!manifest.name) { manifest.name = manifest.plugin_id; changed = true; }
  if (!manifest.version) { manifest.version = '0.1.0'; changed = true; }
  if (!manifest.runtime || typeof manifest.runtime !== 'object') {
    ...
    if (!manifest.runtime.type) { manifest.runtime.type = 'broker-poller'; changed = true; }
    if (!manifest.runtime.url) { manifest.runtime.url = 'http://localhost:8787/funpay-pulse/broker-poller'; changed = true; }
  }
  if (!Array.isArray(manifest.events)) { manifest.events = []; changed = true; }
  if (!Array.isArray(manifest.scopes)) { manifest.scopes = []; changed = true; }
  if (manifest.config_schema && typeof manifest.config_schema !== 'object') { manifest.config_schema = {}; changed = true; }
  if (manifest.ui_schema && typeof manifest.ui_schema !== 'object') { manifest.ui_schema = {}; changed = true; }

  if (changed) await writeJson(manifestPath, manifest);
  return manifest;
}

async function ensureDefaultFixtures(projectRoot: string) {
  const fixturesPath = path.join(projectRoot, 'fixtures', 'events.json');
  const existing = await readJson<any>(fixturesPath, null as any);
...
async function extractZipNode(zipPath: string, dest: string) {
  ...
  for (let i = 0; i < total; i++) {
    if (buf.readUInt32LE(ptr) !== 0x02014b50) throw new Error('ZIP: central directory is corrupted');
    ...
    const name = rawName.toString((flags & 0x800) ? 'utf8' : 'utf8').replace(/\\/g, '/');
    ptr += 46 + nameLen + extraLen + commentLen;

    if (!name || name.startsWith('__MACOSX/') || name.endsWith('/')) continue;
    if (compressedSize === 0xffffffff || localOffset === 0xffffffff) throw new Error('ZIP64 archives are not supported yet');
    ...
    if (buf.readUInt32LE(localOffset) !== 0x04034b50) throw new Error('ZIP: local file header is corrupted');
...
async function extractArchive(src: string, dest: string) {
  const errors: string[] = [];
  try {
  ...
  const py = await spawnCollect(pythonCmd(), ['-c', pyScript, src, dest], STUDIO_ROOT, {}, 120000);
  if (py.success) return;
  errors.push(`Python unzip: ${py.stderr || py.error || 'failed'}`);

  if (process.platform === 'win32') {
    const ps = await spawnCollect('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', 'Expand-Archive -LiteralPath $args[0] -DestinationPath $args[1] -Force', src, dest], STUDIO_ROOT, {}, 120000);
...
function venvPaths(projectRoot: string) {
  const win = process.platform === 'win32';
  return {
    py: path.join(projectRoot, '.venv', win ? 'Scripts/python.exe' : 'bin/python'),
    pulse: path.join(projectRoot, '.venv', win ? 'Scripts/pulse-plugin.exe' : 'bin/pulse-plugin')
  };
}

function isAsarVirtualPath(p: string) {
...
async function copyAsarWheelToRealFile(source: string) {
...
async function resolveInstallableSdkWheel() {
  ...
  const candidates = [
    ...
    { label: 'dist sibling: resources/sdk', path: path.join(__dirname, '..', 'resources', 'sdk', SDK_WHEEL_NAME) }
  ];
...
async function pulseInvocation(projectRoot: string) {
  const { py, pulse } = venvPaths(projectRoot);
  if (await exists(pulse)) return { cmd: pulse, argsPrefix: [] as string[], display: pulse };
  return { cmd: py, argsPrefix: ['-m', 'funpay_pulse_sdk.cli'] as string[], display: `${py} -m funpay_pulse_sdk.cli` };
}

async function runPulse(projectRoot: string, args: string[], env: NodeJS.ProcessEnv = {}, timeoutMs = 120000) {
...
function sdkInstallHint(stderr: string, wheelInfo: any) {
...
function spawnCollect(command: string, args: string[], cwd: string, env: NodeJS.ProcessEnv = {}, timeoutMs = 120000): Promise<any> {
...
async function inferProjectRoot(root: string) {
...
function resolvePreloadPath() {
  // Electron-vite writes preload.js in build/preview, while dev setups and
  // older archives may have slightly different output locations. Searching a
  // small whitelist keeps the renderer bridge reliable instead of silently
  // starting without window.studio.
  const candidates = [
...
function senderWindow(event: any) {
  return BrowserWindow.fromWebContents(event.sender) || win;
}

ipcMain.handle('window:minimize', (event) => { const w = senderWindow(event); w?.minimize(); return true; });
ipcMain.handle('window:maximize', (event) => { const w = senderWindow(event); if (!w) return false; w.maximize(); return true; });
ipcMain.handle('window:unmaximize', (event) => { const w = senderWindow(event); if (!w) return false; w.unmaximize(); return true; });
ipcMain.handle('window:maximizeToggle', (event) => { const w = senderWindow(event); if (!w) return false; w.isMaximized() ? w.unmaximize() : w.maximize(); return w.isMaximized(); });
ipcMain.handle('window:close', (event) => { const w = senderWindow(event); w?.close(); return true; });
ipcMain.handle('window:isMaximized', (event) => { const w = senderWindow(event); return w?.isMaximized() || false; });
...
ipcMain.handle('studio:setSettings', async (_e, s) => { await writeJson(SETTINGS_FILE, s); return true; });
...
ipcMain.handle('updater:setManifestUrl', async (_e, url: string) => {
  const clean = safeUpdateUrl(url);
  await saveSettingsPatch({ updateManifestUrl: clean });
  return { success: true, manifestUrl: clean };
});

ipcMain.handle('updater:checkInno', async (_e, manifestUrl?: string) => {
  const settings = await loadSettings();
...
ipcMain.handle('updater:downloadInno', async (_e, manifest: any) => {
  const result = await downloadInnoUpdate(manifest);
  return result;
});

ipcMain.handle('updater:installInno', async (_e, installerPath?: string) => installDownloadedInnoUpdate(installerPath));
...
ipcMain.handle('auth:telegramPoll', async (_e, code: string) => {
...
ipcMain.handle('developerTokens:revoke', async (_e, publicId: string) => {
...
ipcMain.handle('studio:securityScan', async (_e, projectId: string) => scanProjectSecurity(projectId));
ipcMain.handle('studio:packageGuard', async (_e, projectId: string) => packageGuard(projectId));
...
ipcMain.handle('auth:openAuthorProfile', async () => {
  ...
  if (!response.ok) {
    ...
    if (cachedSlug) {
      const url = `${PULSE_API_BASE}/plugins/authors/${encodeURIComponent(cachedSlug)}`;
      await shell.openExternal(url);
      return { success: true, cached: true, slug: cachedSlug, url, warning: 'Не удалось обновить профиль разработчика, открыт сохранённый slug.' };
    }
  ...
  // Current API shape:
  // { profile: { slug: "andreycatser", ... } }
  // Keep top-level fallback for older/dev servers.
...
ipcMain.handle('auth:logout', async () => {
  try { await pulseApi('/api/v2/auth/logout', { method: 'POST' }); } catch {}
...
ipcMain.handle('studio:listProjects', async () => {
  ...
  for (const e of await fs.readdir(STUDIO_ROOT, { withFileTypes: true })) {
    ...
    const manifest = await readJson(path.join(actualRoot, 'funpay-pulse.plugin.json'), null as any);
...
ipcMain.handle('studio:createEntry', async (_e, projectId, rel, type) => {
  ...
  const full = path.join(actualRoot, rel);
  if (!isInside(actualRoot, full)) throw new Error('Invalid path');
  if (type === 'directory') await ensureDir(full); else { await ensureDir(path.dirname(full)); await fs.writeFile(full, '', 'utf8'); }
  return true;
});
...
ipcMain.handle('studio:openPath', async (_e, p) => { await shell.openPath(p); return true; });

async function ensureSdkForProject(projectId: string) {
  const { actualRoot } = await getProject(projectId);
  await repairLegacyManifest(actualRoot);
  await ensureDefaultFixtures(actualRoot);
  ...
  const ensurePip = await spawnCollect(py, ['-m', 'ensurepip', '--upgrade'], actualRoot, {}, 120000);
  if (!ensurePip.success) {
    const result = { ...ensurePip, step: 'ensurepip', python: py, hint: 'В .venv не удалось включить pip. Проверь установку Python и модуль venv.' };
    await appendSessionEvent(projectId, { type: 'sdk', title: 'ensure SDK', result });
    return result;
  }
  ...
  if (!wheelInfo.success || !wheelInfo.installPath) {
    const result = { ...wheelInfo, success: false, step: 'find-sdk-wheel', hint: wheelInfo.hint || 'SDK wheel не найден или путь к wheel пустой.' };
    await appendSessionEvent(projectId, { type: 'sdk', title: 'ensure SDK', result });
    return result;
  }
  const installPath = wheelInfo.installPath;

  const pip = await spawnCollect(py, ['-m', 'pip', 'install', '--upgrade', '--no-deps', installPath], actualRoot, {}, 180000);
  if (!pip.success) {
    const result = {
      ...pip,
  ...
  const help = await runPulse(actualRoot, ['--help'], {}, 60000);
  const importCheck = await spawnCollect(py, ['-c', 'import funpay_pulse_sdk; print(funpay_pulse_sdk.__name__)'], actualRoot, {}, 60000);
  const result = {
...
ipcMain.handle('studio:fixSsl', async (_e, projectId) => {
  const { actualRoot } = await getProject(projectId);
  const { py } = venvPaths(actualRoot);
  const r = await spawnCollect(py, ['-c', 'import certifi; print(certifi.where())'], actualRoot, {}, 60000);
...
ipcMain.handle('studio:runCommand', async (_e, projectId, preset, extra) => {
  ...
  const manifest = await readJson<any>(path.join(actualRoot, 'funpay-pulse.plugin.json'), {});
  ...
  const trusted = !!extra?.trusted || (manifest.scopes || []).some((scope: string) => trustedScopes.includes(scope));
  const sslEnv = await readJson<Record<string,string>>(path.join(actualRoot, '.plugin-studio-ssl-env.json'), {});
  let cmd = pulse;
  let args: string[] = [];
  ...
  else if (preset === 'run-fixtures') { cmd = py; args = ['app.py', '--fixtures', 'fixtures/events.json']; }
  ...
  if (cmd === pulse && !await exists(pulse)) {
    ...
    if (!setup.success) {
      ...
      await appendSessionEvent(projectId, { type: 'command', title: preset, result });
      return result;
    }
  }
  ...
  let r: any;
  let cleanStaging: any = null;
  if (shouldRunInCleanStaging(String(preset))) {
    const guard = await packageGuard(projectId);
    if (!guard.success) {
      const result = { success: false, step: `clean-staging-before-${preset}`, guard, hint: guard.hint };
      await appendSessionEvent(projectId, { type: 'command', title: preset, result });
      return result;
    }
    ...
    try {
      ...
      r = cmd === pulse
        ? await runPulseUsingVenv(actualRoot, stagingRoot, args, sslEnv, 180000)
        : await spawnCollect(cmd, args, stagingRoot, sslEnv, 180000);
    } finally {
      await fs.rm(stagingParent, { recursive: true, force: true }).catch(() => {});
    }
  } else {
    r = cmd === pulse ? await runPulse(actualRoot, args, sslEnv, 180000) : await spawnCollect(cmd, args, actualRoot, sslEnv, 180000);
  }

  const stderr = r.stderr || r.error || '';
  const hint = stderr.includes('CERTIFICATE_VERIFY_FAILED') ? 'Нажми Исправить SSL / certifi и повтори команду.'
    : stderr.includes('ModuleNotFoundError') ? 'В app.py не хватает Python-зависимости. Studio теперь ставит requirements.txt и частые импорты вроде requests автоматически; если модуль нестандартный — добавь его в requirements.txt.'
    : stderr.includes('hidden files are not allowed') ? 'SDK увидел hidden-файл. В v21 команды запускаются через clean-staging без .plugin-studio/dist/.venv; повтори команду после обновления.'
    : stderr.includes('trusted plugin review') ? 'Плагин просит trusted-права. Studio добавляет --trusted автоматически; проверь manifest.'
    : stderr.includes('No module named pip') ? 'В .venv нет pip. Нажми Установить SDK, Studio попробует ensurepip.'
    : stderr.includes('manifest_version') || stderr.includes('plugin_id') ? 'Manifest был приведён к SDK v1 перед запуском. Если ошибка осталась — открой funpay-pulse.plugin.json и проверь обязательные поля manifest_version/plugin_id/name/version/runtime.'
    : stderr.includes('fixtures: invalid') ? 'fixtures/events.json должен быть в формате SDK: объект с items[]. Studio создаёт валидный fixture для новых проектов.'
    : stderr.includes('app.asar') || stderr.includes('file does not exist') ? 'Команда увидела путь внутри app.asar. В v10 SDK wheel копируется во внешнюю папку; нажми “Установить SDK” и повтори команду.'
    : stderr.includes('ENOENT') ? 'Исполняемый файл не найден. Нажми “Установить SDK” и повтори команду.'
    : r.killed ? 'Команда превысила лимит времени. Проверь, не завис ли app.py или сеть.'
    : '';
...
ipcMain.handle('studio:getManifest', async (_e, projectId) => {
  const { actualRoot } = await getProject(projectId);
  await repairLegacyManifest(actualRoot);
  return await readJson(path.join(actualRoot, 'funpay-pulse.plugin.json'), {} as any);
});
...
ipcMain.handle('studio:packageInfo', async (_e, projectId) => {
  ...
  const rows = [];
  for (const f of files.filter(f => f.endsWith('.fppkg'))) {
...
ipcMain.handle('studio:publishPackage', async (_e, projectId, payload) => {
  ...
  const manifest = await readJson<any>(path.join(actualRoot, 'funpay-pulse.plugin.json'), {});
  const trustedScopes = ['messages:send', 'orders:refund', 'orders:review', 'lots:active', 'lots:price', 'lots:raise', 'blacklist:add', 'blacklist:remove'];
  const trusted = !!payload?.trusted || (manifest.scopes || []).some((s: string) => trustedScopes.includes(s));
  ...
  try {
    ...
    const setup = await ensureSdkForProject(projectId);
    if (!setup.success) {
      const result = { success: false, step: 'auto-ensure-sdk-before-publish', setup, hint: setup.hint || 'Перед публикацией нужно установить SDK в .venv.' };
      await appendSessionEvent(projectId, { type: 'publish', title: 'setup before publish', result });
      return result;
    }
    const guard = await packageGuard(projectId);
    if (!guard.success) {
      const result = { success: false, step: 'review-package-guard', guard, hint: guard.hint };
      await appendSessionEvent(projectId, { type: 'publish', title: 'review package guard', result });
      return result;
    }
    ...
    const marketplace = await readJson<any>(path.join(actualRoot, 'funpay-pulse.marketplace.json'), {});
    ...
    const r = await runPulseUsingVenv(actualRoot, stagingRoot, args, sslEnv, 240000);
    await fs.rm(stagingParent, { recursive: true, force: true }).catch(() => {});
    const result = {
      ...r,
      ...
      hint: r.success
        ? 'Пакет отправлен на review через clean-staging: dist, .plugin-studio, .venv, архивы, логи и мусор не попали в review-пакет.'
        : ((r.stderr || '').includes('CERTIFICATE_VERIFY_FAILED') ? 'Нажми Исправить SSL / certifi и повтори публикацию.' : '')
    };
